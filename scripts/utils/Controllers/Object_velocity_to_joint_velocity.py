...
class VelocityMapper:
    ...
    def object_velocity_to_joint_velocities(self, data, object_velocity):
        """
        Convert full 6D object velocity to joint velocities for dual-arm manipulation.
        Uses full Jacobians for compliant motion in all directions.
        """
        try:
            # Find box body ID
            box_body_id = -1
            ...
            if left_ee_site_id == -1 or right_ee_site_id == -1:
                print("⚠️ Could not find both end-effector sites")
                return np.zeros(12)

            # Get joint DOFs per arm
            ...
            # Extract per-arm Jacobians
            jac_left_arm = jac_left[:, left_arm_dofs]
            jac_right_arm = jac_right[:, right_arm_dofs]

            # Combine into one system
            total_dofs = len(left_arm_dofs) + len(right_arm_dofs)
            J_combined = np.zeros((12, total_dofs))
            J_combined[:6, :len(left_arm_dofs)] = jac_left_arm
            J_combined[6:, len(left_arm_dofs):] = jac_right_arm

            # Desired object velocity for both arms
            v_combined = np.concatenate([object_velocity, object_velocity])  # (12,)

            # Damped pseudoinverse (robust)
            lambda_reg = 1e-4
            J_pinv = J_combined.T @ np.linalg.inv(J_combined @ J_combined.T + lambda_reg * np.eye(12))

            # Map to joint velocities
            joint_velocities = J_pinv @ v_combined  # shape: (12,)

            # Scale & limit
            joint_velocities *= 1.0  # Optional gain
