...
class ObjectAdmittanceController:
    ...
    def compute_object_admittance_control(self, data, left_wrench, right_wrench, desired_height=0.5):
        ...
        W = data.cfrc_ext[box_body_id].copy()
        ...
        # --- Friction Compensation (for X and Y axes) ---
        mu = 0.5  # Friction coefficient (could also be passed in)
        normal_force = np.abs(W[2])  # Assuming Z is upward
        friction_comp = mu * normal_force

        # Apply limited frictional support in X and Y directions
        W_desired[0] += np.clip(W[0], -friction_comp, friction_comp)
        W_desired[1] += np.clip(W[1], -friction_comp, friction_comp)
        ...
        K = np.diag([500.0, 500.0, 400.0, 30.0, 30.0, 30.0])  # Stiffness (increased for z-axis)
